...
enum AntigravityUsageMapper {
    ...
    static func parseQuotaSummary(_ data: Data) -> [MetricLine]? {
        ...
        var sawAnyBucket = false
        var mappedAnyBucket = false

        for group in groups {
            let groupWindowHint = [group.displayName, group.description]
                .compactMap { $0?.trimmingCharacters(in: .whitespacesAndNewlines).nilIfEmpty }
                .joined(separator: " ")
            for bucket in group.buckets ?? [] {
                sawAnyBucket = true
                guard let id = bucket.bucketId else { continue }
                guard let fraction = normalizeRemainingFraction(bucket.remainingFraction) else {
                    AppLog.warn(LogTag.plugin("antigravity"), "quota summary: bucket '\(id)' has no usable remainingFraction; dropping its line")
                    continue
                }
                let reset = bucket.resetTime.flatMap { OpenUsageISO8601.date(from: $0) }
                let windowHint = bucket.window ?? groupWindowHint.nilIfEmpty

                if summaryBuckets.contains(where: { $0.bucketID == id }) {
                    if pooled[id] == nil {
                        pooled[id] = (fraction, reset)
                        mappedAnyBucket = true
                    }
                    continue
                }

                guard let targetID = summaryTarget(forPerModelBucketID: id, window: windowHint) else {
                    AppLog.warn(LogTag.plugin("antigravity"), "quota summary: skipping unrecognized bucket id '\(id)'")
                    continue
                }
                mappedAnyBucket = true
                mergeWorst(into: &pooled, key: targetID, fraction: fraction, resetTime: reset)
            }
        }

        if sawAnyBucket && !mappedAnyBucket {
            // New bucket shapes we don't understand yet — let legacy endpoints try rather than
            // returning an authoritative empty summary that blocks them.
            AppLog.warn(LogTag.plugin("antigravity"), "quota summary had buckets but none mapped; treating as not-a-summary")
            return nil
        }
    ...
    /// Maps a per-model `bucketId` (e.g. `gemini-3.5-flash-low`, `claude-sonnet-4-6`) into one of the
    /// four merged pool keys using its `window` (or the parent group's display name as a fallback).
    static func summaryTarget(forPerModelBucketID id: String, window: String?) -> String? {
        let poolPrefix: String
        if id.hasPrefix("gemini-") {
            poolPrefix = "gemini"
        } else if id.hasPrefix("claude-") || id.hasPrefix("gpt-") {
            poolPrefix = "3p"
        } else {
            return nil
        }
        let windowKind = inferWindowKind(from: window) ?? .session
        switch (poolPrefix, windowKind) {
        case ("gemini", .session): return "gemini-5h"
        case ("gemini", .weekly): return "gemini-weekly"
        case ("3p", .session): return "3p-5h"
        case ("3p", .weekly): return "3p-weekly"
        default: return nil
        }
    }
    ...
    private static func inferWindowKind(from text: String?) -> WindowKind? {
        guard let raw = text?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased(), !raw.isEmpty else {
            return nil
        }
        if raw.contains("week") { return .weekly }
        if raw.contains("5h") || raw.contains("5-hour") || raw.contains("5 hour") || raw.contains("session") {
            return .session
        }
        return nil
    }

    private static func mergeWorst(
        into pooled: inout [String: (fraction: Double, resetTime: Date?)],
        key: String,
        fraction: Double,
        resetTime: Date?
    ) {
        if let existing = pooled[key] {
            if fraction < existing.fraction {
                pooled[key] = (fraction, resetTime)
            }
        } else {
            pooled[key] = (fraction, resetTime)
        }
    ...
    /// Cloud Code sometimes sends 0…1 fractions; tolerate accidental 0…100 percent payloads.
    static func normalizeRemainingFraction(_ raw: Double?) -> Double? {
        guard let raw, raw.isFinite else { return nil }
        if raw > 100 { return nil }
...
private struct QuotaSummaryBucket: Decodable {
    ...
    private enum CodingKeys: String, CodingKey { case bucketId, remainingFraction, resetTime, window, displayName }
    ...
    init(from decoder: Decoder) {
        ...
        window = container.flatMap { (try? $0.decodeIfPresent(String.self, forKey: .window)) ?? nil }
            ?? container.flatMap { (try? $0.decodeIfPresent(String.self, forKey: .displayName)) ?? nil }
    }
}
