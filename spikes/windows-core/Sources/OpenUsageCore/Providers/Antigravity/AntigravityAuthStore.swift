...
/// Credentials Antigravity / the `agy` CLI already stored on the machine. On current builds the OAuth
/// tokens live in the OS keyring (service `gemini`, account `antigravity`) as a `go-keyring-base64`-wrapped
/// JSON blob holding an access token, a refresh token, and an expiry — written by the Antigravity app or
/// `agy`. On Windows that is Credential Manager target `gemini:antigravity` (LegacyGeneric).
...
struct AntigravityAuthStore: Sendable {
    ...
    /// lifetime instead of every refresh cycle. We never write back to Antigravity's keyring item.
    static var cachePath: String {
        WellKnownPaths.localAppData
            .appendingPathComponent("OpenUsage/antigravity/auth.json", isDirectory: false)
            .path
    }
    ...
    init(
        keychain: KeychainAccessing = WindowsCredentialVaultAccessor(),
    ...
    /// Decode the keyring value into tokens. Mirrors the `agy` format: an optional
